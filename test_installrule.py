...
def tcp_tap(inport,outport,tapdst):
    path = shortestpath(inport,outport,weight=defaultwight)
    pathmap = dict(path)
    tap_path = shortestpath_tap(tapdst,pathmap.keys())
    tapport = tap_path[0]
    tapnode = tapport[0]
...
def pre_generate_flows():
    ...
    for ip1,port1 in items:
        for ip2,port2 in items:
            if ip1!=ip2:
                path = shortestpath(port1,port2,weight=dummy_weight)
    ...
    for ip1,port1 in items:
        for ip2,port2 in items:
            if ip1!=ip2:
                pathmap = tcp_tap(port1,port2,(1,4))
...
def pre_read_flows():
    ...
    Match = ofp.OFPMatch
    ActionOutput = ofp.OFPActionOutput
    fp=open('gen/rules1.txt')
    alltable={}
    for line in fp.readlines():
        id,srcip,dstip,ports=line.split()
        id_num = int(id[1:])+1
        table = alltable.get(id_num)
        if table==None:
            table=[]
            alltable[id_num]=table
        ipMatch=Match(eth_type=0x0800,ipv4_src=srcip,ipv4_dst=dstip)
        portnums = [int(pstr) for pstr in ports.split(',')]
        action = [ActionOutput(p) for p in portnums]
        table.append((1,ipMatch,action))
    return alltable
...
if __name__ == '__main__':
    sys.argv.append('test_installrule.py')
